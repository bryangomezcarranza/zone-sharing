...
@MainActor
final class ViewModelTwo: ObservableObject {
    ...
    enum State {
        case loading
        case loaded(private: [Post], shared: [Post])
        case error(Error)
    }
    ...
    let recordZone = CKRecordZone(zoneName: "Post")
    ...
    func fetchPrivateAndSharedContacts() async throws -> (private: [Post], shared: [Post]) {
    ...
    func fetchOrCreateShare(contact: Post) async throws -> (CKShare, CKContainer) {
        guard let existingShare = contact.associatedRecord.share else {
            let share = CKShare(rootRecord: contact.associatedRecord)
            share[CKShare.SystemFieldKey.title] = "Post: \(contact.message)"
    ...
    private func fetchContacts(
        ...
        var allContacts: [Post] = []

        // Inner function retrieving and converting all Contact records for a single zone.
        @Sendable func contactsInZone(_ zone: CKRecordZone) async throws -> [Post] {
            var allContacts: [Post] = []
    ...
    private func fetchSharedContacts() async throws -> [Post] {
